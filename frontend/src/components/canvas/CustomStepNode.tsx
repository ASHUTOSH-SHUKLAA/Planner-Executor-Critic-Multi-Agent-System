...
export const CustomStepNode = memo(({ data }: NodeProps<CustomStepNodeType>) => {
  ...
  const getStatusBadge = () => {
    switch (step.status) {
      ...
      case "EXECUTING":
        return (
          <span className="flex items-center gap-1 rounded-full bg-sky-500/10 border border-sky-500/30 px-2 py-0.5 text-[10px] font-semibold text-sky-400">
      ...
      case "CRITIC_REVIEW":
        return (
          <span className="flex items-center gap-1 rounded-full bg-purple-500/10 border border-purple-500/30 px-2 py-0.5 text-[10px] font-semibold text-purple-400">
      ...
      case "REPLANNED":
        return (
          <span className="flex items-center gap-1 rounded-full bg-cyan-500/10 border border-cyan-500/30 px-2 py-0.5 text-[10px] font-semibold text-cyan-400">
      ...
      case "FAILED":
        return (
          <span className="flex items-center gap-1 rounded-full bg-red-500/10 border border-red-500/30 px-2 py-0.5 text-[10px] font-semibold text-red-400">
      ...
      default:
        return (
          <span className="flex items-center gap-1 rounded-full bg-zinc-800 border border-zinc-700 px-2 py-0.5 text-[10px] font-medium text-zinc-400">
  ...
  const getBorderColor = () => {
    if (isSelected) return "border-indigo-500 ring-2 ring-indigo-500/40 shadow-indigo-500/20";
    switch (step.status) {
      case "PASSED":
        return "border-emerald-500/30 hover:border-emerald-500/60";
      case "EXECUTING":
        return "border-sky-500/50 hover:border-sky-500 shadow-lg shadow-sky-500/10";
      case "CRITIC_REVIEW":
        return "border-purple-500/50 hover:border-purple-500 shadow-lg shadow-purple-500/10";
      case "RETRYING":
  ...
  return (
    <div
      ...
      className={`w-64 rounded-xl border bg-zinc-900/90 p-3.5 shadow-xl backdrop-blur-md cursor-pointer transition-all duration-200 ${getBorderColor()}`}
    >
      ...
      {/* Title */}
      <h6 className="text-xs font-semibold text-zinc-100 line-clamp-2 leading-snug mb-2">
        {step.title}
      </h6>
      ...
      {step.critic_review && (
        <div className="mt-2.5 pt-2 border-t border-zinc-800/80 flex items-center justify-between text-[10px] font-mono">
          <span
            className={`${
              step.critic_review.correctness >= 0.85 ? "text-emerald-400" : "text-amber-400"
            }`}
            title="Correctness"
          >
          ...
          <span
            className={`${
              step.critic_review.completeness >= 0.80 ? "text-emerald-400" : "text-amber-400"
            }`}
            title="Completeness"
          >
          ...
          <span
            className={`${
              step.critic_review.relevance >= 0.85 ? "text-emerald-400" : "text-amber-400"
            }`}
            title="Relevance"
          >
