...
import { Bot, Sparkles, LogOut, ArrowRight, LayoutDashboard } from "lucide-react";
    ...
    <header className="sticky top-0 z-50 w-full border-b border-white/5 bg-[#09090b]/80 backdrop-blur-md">
      <div className="max-w-7xl mx-auto flex h-16 items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <Link href="/" className="flex items-center gap-2.5 group">
          <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 text-white shadow-md shadow-indigo-500/20 group-hover:scale-105 transition-transform">
            <Bot className="h-5 w-5" />
          </div>
          <div className="flex flex-col">
            <div className="flex items-center gap-1.5">
              <span className="text-base font-bold tracking-tight text-white">AegisAgent</span>
              <span className="rounded-full bg-indigo-500/10 border border-indigo-500/20 px-1.5 py-0.2 text-[10px] font-medium text-indigo-400">
                Triad Engine
              </span>
            </div>
            <span className="text-[10px] text-zinc-400 -mt-0.5">Planner • Executor • Critic</span>
          </div>
        </Link>

        {/* Navigation Links */}
        <nav className="hidden md:flex items-center gap-8 text-sm font-medium text-zinc-400">
          <Link href="/#triad" className="hover:text-white transition-colors">
            Triad System
          </Link>
        ...
        <div className="flex items-center gap-3">
          {isAuthenticated && user ? (
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <Link
                href="/signup"
                className="flex items-center gap-1.5 rounded-lg bg-white px-3.5 py-2 text-xs font-semibold text-zinc-950 shadow-sm hover:bg-zinc-200 transition-colors"
              >
